...
def shuffled_middle_word(m):
    """
    This function takes a result of string patterning consisting of groups (e.g. beginning of string + middle + end), shuffles the second group 
    and returns a string joining 1 group + shuffled 2nd group + 3rd group. Intended use: to get a string with shuffled middle.
        INPUT AND OUTPUT:
    ...
    #(if necessary, the segmentation of a word can be changed to any number of segments (not necessarily 3); in that case, 
    # the pattern in shuffle_middles_re should be changed accordingly)
...
def shuffled_word(m):
    ...
    random.shuffle(word)
    return "".join(word)
    #(if necessary, this function can be changed to perform other operations instead of shuffling by writing something else instead of random.shuffle(word). 
    # Results of main functions shuffle_words_re and shuffle_short_words_re will change accordingly)
...
def shuffle_middles_re(text):
    ...
    # (if necessary, the pattern first letter + middle + last letter (\w)(\w+)(\w) can be substituted by any other pattern: with a different number of 
    # initial and/or end letters, different word segmentation (e.g.in half) or a different sentence segmentation)
...
def shuffle_short_words_re(text, n):
    ...
    # (if necessary, instead of an upper limit %s in {,%s} one can set a lower limit {%s,} to scramble words longer than n, 
    # or both limits to scramble words between certain lengths)
